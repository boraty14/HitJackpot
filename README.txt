...
spin results and the results will be generated, stored in "Spin Result List" scriptable object
and printed to console. During play, results will be indexed from this result list. If it is not created, it will be when game
...
index. If there is no save file, it uses the one we created from editor and if we did not, it generates one itself when the game starts.
...
"Assets/Game/Scripts/SpinData.cs".
